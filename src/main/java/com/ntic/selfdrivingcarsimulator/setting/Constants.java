package com.ntic.selfdrivingcarsimulator.setting;

public class Constants{


    public  static final int HEIGHT_RESOLUTION = 720;
    public  static final int WEIGHT_RESOLUTION = 1280;


    public  static final int VCAR_DETETION = 20;//PIXL
    public static final int CHANGING_LIGHT_COLOR_TIME = 5000; // mille second
    public static final int TIME_TO_REACTIVATE_SENSOR_OF_LIGHT_CHECKING = 700; // MS
    public static final int TIME_TO_REACTIVATE_SENSOR_OF_PLAQUE_CHECKING = 3700;// mille second


    public static final int CAR_DEFAULT_SPEED = 1;// 100KM/S
    public static final int CAR_MAX_PETROL_TANK = 4000;// 100KM/S

    public static final int CAR_MIN_PETROL_TANK_TO_GO_CHARGE = CAR_MAX_PETROL_TANK *20/100;// 100KM/S

    public static final int TIME_TO_ADD_ONE_UNIT_OF_FUEL =5;




}
